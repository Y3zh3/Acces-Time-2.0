-- Migración manual para renombrar tablas y columnas a español
-- IMPORTANTE: Ejecutar este script directamente en MySQL

-- PASO 1: Eliminar foreign keys temporalmente
ALTER TABLE `face_biometrics` DROP FOREIGN KEY IF EXISTS `face_biometrics_employeeId_fkey`;
ALTER TABLE `face_biometrics` DROP FOREIGN KEY IF EXISTS `face_biometrics_transportPersonnelId_fkey`;
ALTER TABLE `face_biometrics` DROP FOREIGN KEY IF EXISTS `face_biometrics_providerPersonnelId_fkey`;
ALTER TABLE `access_logs` DROP FOREIGN KEY IF EXISTS `access_logs_employeeId_fkey`;
ALTER TABLE `access_logs` DROP FOREIGN KEY IF EXISTS `access_logs_transportId_fkey`;
ALTER TABLE `temporary_passes` DROP FOREIGN KEY IF EXISTS `temporary_passes_issuedById_fkey`;

-- PASO 2: Renombrar tablas
ALTER TABLE `employees` RENAME TO `empleados`;
ALTER TABLE `face_biometrics` RENAME TO `biometricos_faciales`;
...
-- PASO 3: Renombrar columnas de empleados
...
SELECT 'Migración completada - Todas las tablas y columnas ahora están en español' AS mensaje;
